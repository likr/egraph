...
describe('brandes', () => {
  ...
  const [d1, d2, d3, d4, d5, d6, d7] = [16, 17, 18, 19, 20, 21, 22]
  const [e1, e2, e3] = [23, 24, 25]
  const layers = [
    [a1, a2],
  ...
  describe('verticalAlignment(g, layers, {rtol: false, btot: true})', () => {
    it('set root and align for all vertices', () => {
      const graph = exampleGraph()
      verticalAlignment(graph, layers, {rtol: false, btot: true})
      assert.equal(graph.vertex(a1).root, d5)
      assert.equal(graph.vertex(a1).align, d5)
      assert.equal(graph.vertex(a2).root, a2)
      assert.equal(graph.vertex(a2).align, a2)
      assert.equal(graph.vertex(b1).root, b1)
      assert.equal(graph.vertex(b1).align, b1)
      assert.equal(graph.vertex(b2).root, c2)
      assert.equal(graph.vertex(b2).align, c2)
      assert.equal(graph.vertex(b3).root, b3)
      assert.equal(graph.vertex(b3).align, b3)
      assert.equal(graph.vertex(b4).root, b4)
      assert.equal(graph.vertex(b4).align, b4)
      assert.equal(graph.vertex(b5).root, e3)
      assert.equal(graph.vertex(b5).align, e3)
      assert.equal(graph.vertex(b6).root, d5)
      assert.equal(graph.vertex(b6).align, a1)
      assert.equal(graph.vertex(b7).root, d7)
      assert.equal(graph.vertex(b7).align, d7)
      assert.equal(graph.vertex(b8).root, b8)
      assert.equal(graph.vertex(b8).align, b8)
      assert.equal(graph.vertex(c1).root, e2)
      assert.equal(graph.vertex(c1).align, e2)
      assert.equal(graph.vertex(c2).root, c2)
      assert.equal(graph.vertex(c2).align, b2)
      assert.equal(graph.vertex(c3).root, e3)
      assert.equal(graph.vertex(c3).align, b5)
      assert.equal(graph.vertex(c4).root, d5)
      assert.equal(graph.vertex(c4).align, b6)
      assert.equal(graph.vertex(c5).root, d6)
      assert.equal(graph.vertex(c5).align, d6)
      assert.equal(graph.vertex(c6).root, d7)
      assert.equal(graph.vertex(c6).align, b7)
      assert.equal(graph.vertex(d1).root, e1)
      assert.equal(graph.vertex(d1).align, e1)
      assert.equal(graph.vertex(d2).root, e2)
      assert.equal(graph.vertex(d2).align, c1)
      assert.equal(graph.vertex(d3).root, d3)
      assert.equal(graph.vertex(d3).align, d3)
      assert.equal(graph.vertex(d4).root, e3)
      assert.equal(graph.vertex(d4).align, c3)
      assert.equal(graph.vertex(d5).root, d5)
      assert.equal(graph.vertex(d5).align, c4)
      assert.equal(graph.vertex(d6).root, d6)
      assert.equal(graph.vertex(d6).align, c5)
      assert.equal(graph.vertex(d7).root, d7)
      assert.equal(graph.vertex(d7).align, c6)
      assert.equal(graph.vertex(e1).root, e1)
      assert.equal(graph.vertex(e1).align, d1)
      assert.equal(graph.vertex(e2).root, e2)
      assert.equal(graph.vertex(e2).align, d2)
      assert.equal(graph.vertex(e3).root, e3)
      assert.equal(graph.vertex(e3).align, d4)
    })
  })

  describe('horizontalCompaction(g, layers, {rtol: false, btot: false})', () => {
    it('set x for all vertices', () => {
      const direction = {rtol: false, btot: false}
      const graph = exampleGraph()
      verticalAlignment(graph, layers, direction)
      horizontalCompaction(graph, layers, direction)
  ...
  describe('horizontalCompaction(g, layers, {rtol: true, btot: false})', () => {
    it('set x for all vertices', () => {
      const direction = {rtol: true, btot: false}
      const graph = exampleGraph()
      verticalAlignment(graph, layers, direction)
      horizontalCompaction(graph, layers, direction)
      assert.equal(graph.vertex(a1).x, 10)
      assert.equal(graph.vertex(a2).x, 0)
      assert.equal(graph.vertex(b1).x, 90)
      assert.equal(graph.vertex(b2).x, 80)
      assert.equal(graph.vertex(b3).x, 70)
      assert.equal(graph.vertex(b4).x, 60)
      assert.equal(graph.vertex(b5).x, 50)
      assert.equal(graph.vertex(b6).x, 40)
      assert.equal(graph.vertex(b7).x, 20)
      assert.equal(graph.vertex(b8).x, 10)
      assert.equal(graph.vertex(c1).x, 70)
      assert.equal(graph.vertex(c2).x, 60)
      assert.equal(graph.vertex(c3).x, 50)
      assert.equal(graph.vertex(c4).x, 40)
      assert.equal(graph.vertex(c5).x, 30)
      assert.equal(graph.vertex(c6).x, 20)
      assert.equal(graph.vertex(d1).x, 80)
      assert.equal(graph.vertex(d2).x, 70)
      assert.equal(graph.vertex(d3).x, 60)
      assert.equal(graph.vertex(d4).x, 50)
      assert.equal(graph.vertex(d5).x, 40)
      assert.equal(graph.vertex(d6).x, 30)
      assert.equal(graph.vertex(d7).x, 20)
      assert.equal(graph.vertex(e1).x, 80)
      assert.equal(graph.vertex(e2).x, 70)
      assert.equal(graph.vertex(e3).x, 30)
    })
  })

  describe('horizontalCompaction(g, layers, {rtol: false, btot: true})', () => {
    it('set x for all vertices', () => {
      const direction = {rtol: false, btot: true}
      const graph = exampleGraph()
      verticalAlignment(graph, layers, direction)
      horizontalCompaction(graph, layers, direction)
      assert.equal(graph.vertex(a1).x, 60)
      assert.equal(graph.vertex(a2).x, 70)
      assert.equal(graph.vertex(b1).x, 10)
      assert.equal(graph.vertex(b2).x, 20)
      assert.equal(graph.vertex(b3).x, 30)
      assert.equal(graph.vertex(b4).x, 40)
      assert.equal(graph.vertex(b5).x, 50)
      assert.equal(graph.vertex(b6).x, 60)
      assert.equal(graph.vertex(b7).x, 80)
      assert.equal(graph.vertex(b8).x, 90)
      assert.equal(graph.vertex(c1).x, 10)
      assert.equal(graph.vertex(c2).x, 20)
      assert.equal(graph.vertex(c3).x, 50)
      assert.equal(graph.vertex(c4).x, 60)
      assert.equal(graph.vertex(c5).x, 70)
      assert.equal(graph.vertex(c6).x, 80)
      assert.equal(graph.vertex(d1).x, 0)
      assert.equal(graph.vertex(d2).x, 10)
      assert.equal(graph.vertex(d3).x, 20)
      assert.equal(graph.vertex(d4).x, 50)
      assert.equal(graph.vertex(d5).x, 60)
      assert.equal(graph.vertex(d6).x, 70)
      assert.equal(graph.vertex(d7).x, 80)
      assert.equal(graph.vertex(e1).x, 0)
      assert.equal(graph.vertex(e2).x, 10)
      assert.equal(graph.vertex(e3).x, 50)
    })
  })

  describe('horizontalCompaction(g, layers, {rtol: true, btot: true})', () => {
    it('set x for all vertices', () => {
      const direction = {rtol: true, btot: true}
      const graph = exampleGraph()
      verticalAlignment(graph, layers, direction)
      horizontalCompaction(graph, layers, direction)
      assert.equal(graph.vertex(a1).x, 50)
      assert.equal(graph.vertex(a2).x, 40)
      assert.equal(graph.vertex(b1).x, 80)
      assert.equal(graph.vertex(b2).x, 70)
      assert.equal(graph.vertex(b3).x, 60)
      assert.equal(graph.vertex(b4).x, 50)
      assert.equal(graph.vertex(b5).x, 40)
      assert.equal(graph.vertex(b6).x, 30)
      assert.equal(graph.vertex(b7).x, 20)
      assert.equal(graph.vertex(b8).x, 10)
      assert.equal(graph.vertex(c1).x, 60)
      assert.equal(graph.vertex(c2).x, 50)
      assert.equal(graph.vertex(c3).x, 40)
      assert.equal(graph.vertex(c4).x, 30)
      assert.equal(graph.vertex(c5).x, 10)
      assert.equal(graph.vertex(c6).x, 0)
      assert.equal(graph.vertex(d1).x, 70)
      assert.equal(graph.vertex(d2).x, 60)
      assert.equal(graph.vertex(d3).x, 50)
      assert.equal(graph.vertex(d4).x, 40)
      assert.equal(graph.vertex(d5).x, 30)
      assert.equal(graph.vertex(d6).x, 10)
      assert.equal(graph.vertex(d7).x, 0)
      assert.equal(graph.vertex(e1).x, 50)
      assert.equal(graph.vertex(e2).x, 10)
      assert.equal(graph.vertex(e3).x, 0)
    })
  })
